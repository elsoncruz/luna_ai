...
<style>
    ...
    [data-testid="stAppViewContainer"]{
        background-image:url("app/static/09.jpg");
...
for message in st.session_state.messages:
    with st.markdown('<div class="message-box">{}</div>'.format(message["role"]), unsafe_allow_html=True):
        if message["role"]=="user":
            st.markdown('<div class="cus"><img src="app/static/user.png" class="img2"></div>''<div class="user-box" style="margin-right: auto;">{}</div>'.format(message["content"]), unsafe_allow_html=True)
            pass
        else:
...
if prompt := st.chat_input("say"):
    st.chat_message("user").markdown('<div class="cus"><img src="app/static/user.png" class="img2"></div>''<div class="user-box">{}</div>'.format(prompt), unsafe_allow_html=True)
    ...
    with st.chat_message("🤖"):
        st.markdown('<img src="app/static/chatbot.png" width=32 height=32>''<div class="message-box">{}</div>'.format(response), unsafe_allow_html=True)
