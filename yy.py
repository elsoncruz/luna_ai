...
<style>
    ...
    [data-testid="stAppViewContainer"]{
        background-image:url("luna_ai/09.jpg");
...
for message in st.session_state.messages:
    with st.markdown('<div class="message-box">{}</div>'.format(message["role"]), unsafe_allow_html=True):
        if message["role"]=="user":
            st.markdown('<div class="cus"><img src="luna_ai/user.png" class="img2"></div>''<div class="user-box" style="margin-right: auto;">{}</div>'.format(message["content"]), unsafe_allow_html=True)
            pass
        else:
            st.markdown('<img src="luna_ai/chatbot.png" width=32 height=32>''<div class="message-box">{}</div>'.format(message["content"]), unsafe_allow_html=True)   
            pass


if prompt := st.chat_input("say"):
    st.chat_message("user").markdown('<div class="cus"><img src="luna_ai/user.png" class="img2"></div>''<div class="user-box">{}</div>'.format(prompt), unsafe_allow_html=True)
    ...
    with st.chat_message("🤖"):
        st.markdown('<img src="luna_ai/chatbot.png" width=32 height=32>''<div class="message-box">{}</div>'.format(response), unsafe_allow_html=True)
